...
def find_latest_cache() -> Path | None:
    # Cache files live in the project root cache/, not tools/cache/
    project_root = Path(__file__).parent.parent
    caches = sorted(project_root.glob("cache/debug_context_*.json"), reverse=True)
...
def cmd_debug_player(context: dict, name: str):
    ...
    for player, sig in matches:
        ...
        signal_keys = [
            ...
            ("pitcher_fb_pct",        "Pitcher FB% (fastball)"),
            ("pitcher_breaking_pct",  "Pitcher breaking%"),
            ("pitcher_offspeed_pct",  "Pitcher offspeed%"),
...
def cmd_pipeline(context: dict):
    ...
    keys = [
        ...
        "pitcher_hr_per_9", "pitcher_fb_pct", "pitcher_breaking_pct", "pitcher_offspeed_pct",
        "h2h_hr", "ev_10", "value_edge", "pinnacle_odds",
