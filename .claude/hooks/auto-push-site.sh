...
CHANGED=$(git status --porcelain 2>/dev/null | grep -E "(picks/.*\.(html|txt)|docs/index\.html|docs/strikeouts\.html)" | awk '{print $2}')
...
git add picks/*.html picks/*.txt docs/index.html docs/strikeouts.html 2>/dev/null
